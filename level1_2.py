...
def RunToGoal(surface, screen, clock, seeker, hider, block, curX, curY, check):
    while curX < check[0]:
        curX+=1
...
def DFSRandom(board, curX, curY, directX, directY, count, surface, screen, clock, seeker, hider, check, block):
    seeker.setPosition(curX, curY)
    updateGameState(surface, screen, clock, seeker, hider, block)
    check = checkVision(curX, curY, board)
    print(check)
    if check[0] != -1:
        print("Run to goal")
        RunToGoal(surface, screen, clock, seeker, hider, block, curX, curY, check)
        print("Finish")
        GameOver()
        ...
        for i in range(8):
            ...
            if newX < SCREEN_WIDTH/GRIDSIZE and newY < SCREEN_HEIGHT/GRIDSIZE and newX >= 0 and newY >= 0:
                if board[newY][newX] == -1:
                    ...
                    # count[0]+=1
                    goal = [newX, newY]
                    RunToGoal(surface, screen, clock, seeker, hider, block, curX, curY, goal)
...
def checkVision(curX, curY, board):
    directX = [-1, 0, 1, 0, -1, 1, 1, -1]
    directY = [0, -1, 0, 1, -1, -1, 1, 1]
    for i in range(3):
        for j in range(8):
            newX = curX + directX[j]
            newY = curY + directY[j]
